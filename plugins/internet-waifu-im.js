...
let kys = x.nsfw
					...
					Object.keys(ky, kys).map((v, index) => {
						arr.push(['Result: ' + ++index, [
							[`Versatile: ${ky[v]}` ? `Versatile: ${ky[0]}` : 'Kosong', usedPrefix + command + ' waifuimget ' + 'https://api.waifu.im/random/?selected_tags=' + ky[v] ? ky[0] : 'undefined', '\n⌚ *Request:* ' + name],
							[`NSFW: ${kys[v]}` ? `NSFW: ${kys[0]}` : 'Kosong', usedPrefix + command + ' waifuimget ' + 'https://api.waifu.im/random/?selected_tags=' + kys[v] ? kys[0] : 'undefined', '\n⌚ *Request:* ' + name]
						]])
					})
